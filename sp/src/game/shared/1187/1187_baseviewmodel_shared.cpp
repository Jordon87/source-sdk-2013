...
void CBaseViewModel::CalcViewModelView(CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles)
{
	...
	CBase1187CombatWeapon *p1187Weapon = ToBase1187CombatWeapon( m_hWeapon.Get() );
	if (p1187Weapon)
	{
		if(!p1187Weapon->IsIronsighted())
			CalcViewModelLag(vmorigin, vmangles, vmangoriginal);
	}
	else
		CalcViewModelLag(vmorigin, vmangles, vmangoriginal);
...
{
	CBase1187CombatWeapon *pWeapon = ToBase1187CombatWeapon( GetOwningWeapon() );
