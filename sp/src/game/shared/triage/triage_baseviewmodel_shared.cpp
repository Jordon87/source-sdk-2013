...
{
	CBaseTriageCombatWeapon *pWeapon = ToBaseTriageCombatWeapon( GetOwningWeapon() );

	if (!pWeapon)
		return;

	//get delta time for interpolation
	float delta = (gpGlobals->curtime - pWeapon->m_flIronsightedTime) * 5.0f; // 2.5f //modify this value to adjust how fast the interpolation is
...
void CBaseViewModel::CalcViewModelView( CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles )
{
	...
	if (pTriageWeapon)
	{
		if (!pTriageWeapon->IsIronsighted())
		{
			CalcViewModelLag(vmorigin, vmangles, vmangoriginal);
		}
	}
