...
#define FRAG_GRACE_TIME_AFTER_PICKUP 1.5f
#define FRAG_WARN_TIME 1.5f
...
LINK_ENTITY_TO_CLASS(npc_grenade_frag_1187, C1187GrenadeFrag);

BEGIN_DATADESC(C1187GrenadeFrag)
...
// Purpose: 
//-----------------------------------------------------------------------------
C1187GrenadeFrag::~C1187GrenadeFrag(void)
{
}

void C1187GrenadeFrag::Spawn(void)
{
	Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void C1187GrenadeFrag::OnRestore(void)
{
	BaseClass::OnRestore();
}

bool C1187GrenadeFrag::CreateVPhysics()
{
...
void C1187GrenadeFrag::VPhysicsUpdate(IPhysicsObject *pPhysics)
{
...
void C1187GrenadeFrag::Precache(void)
{
...
void C1187GrenadeFrag::SetTimer(float detonateDelay, float warnDelay)
{
	m_flDetonateTime = gpGlobals->curtime + detonateDelay;
	m_flWarnAITime = gpGlobals->curtime + warnDelay;
	SetThink(&C1187GrenadeFrag::DelayThink);
	SetNextThink(gpGlobals->curtime);
}

void C1187GrenadeFrag::DelayThink()
{
...
void C1187GrenadeFrag::SetVelocity(const Vector &velocity, const AngularImpulse &angVelocity)
{
...
int C1187GrenadeFrag::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
...
void C1187GrenadeFrag::InputSetTimer(inputdata_t &inputdata)
{
...
CBaseGrenade *Fraggrenade_Create_1187(const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer)
{
	...
	pGrenade->SetTimer(timer, timer - FRAG_WARN_TIME);
