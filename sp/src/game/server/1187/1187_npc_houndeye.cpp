...
private:
	...
	enum
	{
		SCHED_HOUND_AGITATED = BaseClass::NEXT_SCHEDULE,
	...
	enum
	{
		TASK_HOUND_CLOSE_EYE = BaseClass::NEXT_TASK,
		TASK_HOUND_OPEN_EYE,
	...
	enum
	{
		COND_HOUND_GROUP_ATTACK = BaseClass::NEXT_CONDITION,
...
void CNPC_Houndeye::StartTask(const Task_t *pTask)
{
	...
	case TASK_HOUND_GET_PATH_TO_CIRCLE:
	{
		...
		else
		{
			...
			if (GetNavigator()->SetRadialGoal(vTargetPos, WorldSpaceCenter(), random->RandomInt(50, 500), 90, 175, m_bLoopClockwise))
			{
				TaskComplete();
				return;
			}
