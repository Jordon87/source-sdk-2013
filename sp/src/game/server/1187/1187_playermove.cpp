...
class C1187PlayerMove : public CPlayerMove
{
	DECLARE_CLASS(C1187PlayerMove, CPlayerMove);
public:
	C1187PlayerMove() :
...
static C1187PlayerMove g_PlayerMove;
...
void C1187PlayerMove::SetupMove( CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	// Call the default SetupMove code.
	BaseClass::SetupMove( player, ucmd, pHelper, move );

	// Convert to HL2 data.
	C1187_Player *p1187Player = static_cast<C1187_Player*>(player);
	Assert(p1187Player);

	C1187MoveData *p1187Move = static_cast<C1187MoveData*>( move );
	Assert(p1187Move);

	player->m_flForwardMove = ucmd->forwardmove;
	player->m_flSideMove = ucmd->sidemove;

	p1187Move->m_bIsSprinting = p1187Player->IsSprinting();
...
void C1187PlayerMove::FinishMove( CBasePlayer *player, CUserCmd *ucmd, CMoveData *move )
{
