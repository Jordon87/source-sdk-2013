...
class CTriagePlayerMove : public CPlayerMove
{
	DECLARE_CLASS( CTriagePlayerMove, CPlayerMove );
public:
	CTriagePlayerMove() :
...
static CTriagePlayerMove g_PlayerMove;
...
void CTriagePlayerMove::SetupMove( CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	// Call the default SetupMove code.
	BaseClass::SetupMove( player, ucmd, pHelper, move );

	// Convert to Triage data.
	CTriage_Player *pTriagePlayer = static_cast<CTriage_Player*>( player );
	Assert( pTriagePlayer );

	CTriageMoveData *pTriageMove = static_cast<CTriageMoveData*>( move );
	Assert( pTriageMove );

	player->m_flForwardMove = ucmd->forwardmove;
	player->m_flSideMove = ucmd->sidemove;

	pTriageMove->m_bIsSprinting = pTriagePlayer->IsSprinting();
...
void CTriagePlayerMove::FinishMove( CBasePlayer *player, CUserCmd *ucmd, CMoveData *move )
{
