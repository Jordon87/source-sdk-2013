...
END_DATADESC()
LINK_ENTITY_TO_CLASS(triage_rpg_missile, CTriageMissile);

class CTriageWeaponRPG;


//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CTriageMissile::CTriageMissile()
{
	m_hRocketTrail = NULL;
...
//
//-----------------------------------------------------------------------------
void CTriageMissile::Precache(void)
{
...
//
//-----------------------------------------------------------------------------
void CTriageMissile::Spawn(void)
{
	Precache();
	...
	SetTouch(&CTriageMissile::MissileTouch);

	SetMoveType( MOVETYPE_FLYGRAVITY, MOVECOLLIDE_FLY_BOUNCE );
	SetThink(&CTriageMissile::IgniteThink);
...
//---------------------------------------------------------
//---------------------------------------------------------
void CTriageMissile::Event_Killed(const CTakeDamageInfo &info)
{
...
unsigned int CTriageMissile::PhysicsSolidMaskForEntity(void) const
{ 
...
//---------------------------------------------------------
//---------------------------------------------------------
int CTriageMissile::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
void CTriageMissile::DumbFire(void)
{
	SetThink( NULL );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageMissile::SetGracePeriod(float flGracePeriod)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CTriageMissile::AccelerateThink(void)
{
	Vector vecForward;
	...
	SetThink(&CTriageMissile::SeekThink);
...
//---------------------------------------------------------
//---------------------------------------------------------
void CTriageMissile::AugerThink(void)
{
...
//-----------------------------------------------------------------------------
void CTriageMissile::ShotDown(void)
{
	CEffectData	data;
	...
	SetThink(&CTriageMissile::AugerThink);
...
//-----------------------------------------------------------------------------
void CTriageMissile::DoExplosion(void)
{
	// Explode
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageMissile::Explode(void)
{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CTriageMissile::MissileTouch(CBaseEntity *pOther)
{
	Assert( pOther );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageMissile::CreateSmokeTrail(void)
{
	if ( m_hRocketTrail )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageMissile::IgniteThink(void)
{
	SetMoveType( MOVETYPE_FLY );
	SetModel("models/weapons/w_missile.mdl");
	UTIL_SetSize( this, vec3_origin, vec3_origin );
...
//-----------------------------------------------------------------------------
void CTriageMissile::GetShootPosition(CLaserDot *pLaserDot, Vector *pShootPosition)
{
...
void CTriageMissile::ComputeActualDotPosition(CLaserDot *pLaserDot, Vector *pActualDotPosition, float *pHomingSpeed)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageMissile::SeekThink(void)
{
	...
	if( hl2_episodic.GetBool() )
	{
		...
		{
			// Scare targets
			CSoundEnt::InsertSound(SOUND_DANGER, pBestDot->GetAbsOrigin(), CTriageMissile::EXPLOSION_RADIUS, 0.2f, pBestDot, SOUNDENT_CHANNEL_REPEATED_DANGER, NULL);
		}
	}
...
// Output : CMissile
//-----------------------------------------------------------------------------
CTriageMissile *CTriageMissile::Create(const Vector &vecOrigin, const QAngle &vecAngles, edict_t *pentOwner = NULL)
{
	//CMissile *pMissile = (CMissile *)CreateEntityByName("rpg_missile" );
	CTriageMissile *pMissile = (CTriageMissile *)CBaseEntity::Create("triage_rpg_missile", vecOrigin, vecAngles, CBaseEntity::Instance(pentOwner));
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CUtlVector<CTriageMissile::CustomDetonator_t> CTriageMissile::gm_CustomDetonators;

void CTriageMissile::AddCustomDetonator(CBaseEntity *pEntity, float radius, float height)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTriageMissile::RemoveCustomDetonator(CBaseEntity *pEntity)
{
...
BEGIN_DATADESC( CTriageWeaponRPG )
...
IMPLEMENT_SERVERCLASS_ST(CTriageWeaponRPG, DT_TriageWeaponRPG)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_rpg, CTriageWeaponRPG);

acttable_t	CTriageWeaponRPG::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CTriageWeaponRPG);

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CTriageWeaponRPG::CTriageWeaponRPG()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::Precache(void)
{
	BaseClass::Precache();
	...
	UTIL_PrecacheOther( "triage_rpg_missile" );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::Activate(void)
{
	BaseClass::Activate();
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch( pEvent->event )
	{
		case EVENT_WEAPON_SMG1:
		{
			...
			m_hMissile = CTriageMissile::Create( muzzlePoint, vecAngles, GetOwner()->edict() );		
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::HasAnyAmmo(void)
{
	if ( m_hMissile != NULL )
		return true;
...
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::WeaponShouldBeLowered(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::PrimaryAttack(void)
{
...
// Input  : *pOwner - 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::DecrementAmmo(CBaseCombatCharacter *pOwner)
{
...
// Input  : state - 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::SuppressGuiding(bool state)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::Lower(void)
{
	if ( m_hMissile != NULL )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::ItemPostFrame(void)
{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CTriageWeaponRPG::GetLaserPosition(void)
{
	CreateLaserPointer();
...
// Input  : &vecTarget - 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::UpdateNPCLaserPosition(const Vector &vecTarget)
{
	CreateLaserPointer();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::SetNPCLaserPosition(const Vector &vecTarget)
{ 
...
// Purpose: 
//-----------------------------------------------------------------------------
const Vector &CTriageWeaponRPG::GetNPCLaserPosition(void)
{
...
// Purpose:
//-----------------------------------------------------------------------------
const Vector& CTriageWeaponRPG::GetBulletSpread(void)
{
...
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::IsGuiding(void)
{
	return m_bGuiding;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::Deploy(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::Holster(CBaseCombatWeapon *pSwitchingTo)
{
	//Can't have an active missile out
	if ( m_hMissile != NULL )
		return false;
...
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::StartGuiding(void)
{
...
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::StopGuiding(void)
{
	m_bGuiding = false;
...
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::ToggleGuiding(void)
{
	if ( IsGuiding() )
	{
		StopGuiding();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::Drop(const Vector &vecVelocity)
{
	StopGuiding();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::UpdateLaserPosition(Vector vecMuzzlePos, Vector vecEndPos)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::CreateLaserPointer(void)
{
	if ( m_hLaserDot != NULL )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::NotifyRocketDied(void)
{
	m_hMissile = NULL;
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::Reload(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CTriageWeaponRPG::WeaponLOSCondition(const Vector &ownerPos, const Vector &targetPos, bool bSetConditions)
{
...
// Output : int
//-----------------------------------------------------------------------------
int CTriageWeaponRPG::WeaponRangeAttack1Condition(float flDot, float flDist)
{
	if ( m_hMissile != NULL )
		return 0;
...
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::StartLaserEffects(void)
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
		return;
...
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::StopLaserEffects(void)
{
	if ( m_hLaserBeam != NULL )
	{
...
//-----------------------------------------------------------------------------
void CTriageWeaponRPG::UpdateLaserEffects(void)
{
	if ( !m_bGuiding )
		return;
