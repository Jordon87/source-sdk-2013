...
#define GRENADE_TRIPMINE_MODEL		"models/tripmine/w_tripmine.mdl"
...
LINK_ENTITY_TO_CLASS(npc_tripmine, CHoe_Grenade_Tripmine);

BEGIN_DATADESC(CHoe_Grenade_Tripmine)
...
CHoe_Grenade_Tripmine::CHoe_Grenade_Tripmine()
{
	m_vecDir.Init();
	m_vecEnd.Init();
	m_posOwner.Init();
...
void CHoe_Grenade_Tripmine::Spawn(void)
{
	Precache();
	// motor
	SetMoveType(MOVETYPE_FLY);
	SetSolid(SOLID_BBOX);
	SetModel(GRENADE_TRIPMINE_MODEL);
	...
	SetThink(&CHoe_Grenade_Tripmine::PowerupThink);
...
void CHoe_Grenade_Tripmine::Precache(void)
{
	PrecacheModel(GRENADE_TRIPMINE_MODEL);
...
void CHoe_Grenade_Tripmine::WarningThink(void)
{
	// set to power up
	SetThink(&CHoe_Grenade_Tripmine::PowerupThink);
...
void CHoe_Grenade_Tripmine::PowerupThink(void)
{
...
void CHoe_Grenade_Tripmine::KillBeam(void)
{
	if (m_pBeam)
	{
		UTIL_Remove(m_pBeam);
...
void CHoe_Grenade_Tripmine::MakeBeam(void)
{
	trace_t tr;
	...
	SetThink(&CHoe_Grenade_Tripmine::BeamBreakThink);
	...
	m_pBeam = CBeam::BeamCreate(g_pModelNameLaser, 0.5); // 0.35
	m_pBeam->PointsInit(vecTmpEnd, GetAbsOrigin());
...
void CHoe_Grenade_Tripmine::BeamBreakThink(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CHoe_Grenade_Tripmine::Event_Killed(const CTakeDamageInfo &info)
{
	m_takedamage = DAMAGE_NO;

	SetThink(&CHoe_Grenade_Tripmine::DelayDeathThink);
...
void CHoe_Grenade_Tripmine::DelayDeathThink(void)
{
	KillBeam();
	trace_t tr;
