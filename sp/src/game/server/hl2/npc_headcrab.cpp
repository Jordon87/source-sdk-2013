...
ConVar  sk_headcrab_burst_melee_dmg("sk_headcrab_burst_health", "1", FCVAR_NONE);
ConVar  sk_headcrab_burst_squish_dmg("sk_headcrab_burst_health", "20", FCVAR_NONE);
...
void CBurstHeadcrab::TouchDamage(CBaseEntity* pOther)
{
	if (pOther->IsAlive())
	{
		CTakeDamageInfo info(this, this, sk_headcrab_burst_melee_dmg.GetFloat(), DMG_SLASH);
		pOther->TakeDamage(info);
	}
}

void CBurstHeadcrab::Touch(CBaseEntity* pOther)
{
	if (!m_bMidJump)
	{
		if (GetGroundEntity())
		{
			if (pOther->IsPlayer())
			{
				if (pOther->IsAlive())
				{
					...
					if ((pPlayer->m_nButtons & IN_SPEED) && (pPlayer->m_nButtons & IN_FORWARD))
					{
						Msg("Squish The Fucker!\n");
						CTakeDamageInfo info(pPlayer, pPlayer, sk_headcrab_burst_squish_dmg.GetFloat(), DMG_CRUSH);
						TakeDamage(info);
					}
				}
			}
...
AI_BEGIN_CUSTOM_NPC( npc_headcrab_burst, CBurstHeadcrab )
