...
// Use all the gibs
#define	NUM_ZOMBIE_GIBS_UNIQUE	3
const char *pszZombieGibs_Unique[NUM_ZOMBIE_GIBS_UNIQUE] = {
	"models/gibs/antlion_gib_large_1.mdl",
	"models/gibs/antlion_gib_large_2.mdl",
	"models/gibs/antlion_gib_large_3.mdl"
};

#define	NUM_ZOMBIE_GIBS_MEDIUM	3
const char *pszZombieGibs_Medium[NUM_ZOMBIE_GIBS_MEDIUM] = {
...
#define	NUM_ZOMBIE_GIBS_SMALL	3
const char *pszZombieGibs_Small[NUM_ZOMBIE_GIBS_SMALL] = {
...
ConVar g_zombie_maxgibs( "g_zombie_maxgibs", "1", FCVAR_ARCHIVE );

void CZombieGibManager::LevelInitPreEntity( void )
{
	m_LRU.Purge();
}

CZombieGibManager s_ZombieGibManager( "CZombieGibManager" );

void CZombieGibManager::AddGib( C_BaseEntity *pEntity )
{
	m_LRU.AddToTail( pEntity );
}

void CZombieGibManager::RemoveGib( C_BaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
void CZombieGibManager::Update( float frametime )
{
	if ( m_LRU.Count() < g_zombie_maxgibs.GetInt() )
		 return;
...
class C_ZombieGib : public C_Gib
{
	typedef C_Gib BaseClass;
public:
	
	static C_ZombieGib *CreateClientsideGib( const char *pszModelName, Vector vecOrigin, Vector vecForceDir, AngularImpulse vecAngularImp, float m_flLifetime = DEFAULT_GIB_LIFETIME )
	{
		C_ZombieGib *pGib = new C_ZombieGib;
		...
		s_ZombieGibManager.AddGib( pGib );
	...
	{
		int index = decalsystem->GetDecalIndexForName( "RedBlood" );
...
// Input  : &origin - 
//-----------------------------------------------------------------------------
void FX_ZombieGib( const Vector &origin, const Vector &direction, float scale )
{
	Vector	offset;
	...
	int numGibs = random->RandomInt( 1, NUM_ZOMBIE_GIBS_UNIQUE );
	...
	for ( int i = 0; i < numGibs; i++ )
	{
		...
		C_ZombieGib::CreateClientsideGib( pszZombieGibs_Unique[i], offset, ( direction + RandomVector( -0.8f, 0.8f ) ) * ( 150 * scale ), RandomAngularImpulse( -32, 32 ), 2.0f);
	}

	numGibs = random->RandomInt( 1, NUM_ZOMBIE_GIBS_MEDIUM );
	...
	for ( int i = 0; i < numGibs; i++ )
	{
		...
		C_ZombieGib::CreateClientsideGib( pszZombieGibs_Medium[i], offset, ( direction + RandomVector( -0.8f, 0.8f ) ) * ( 250 * scale ), RandomAngularImpulse( -200, 200 ), 1.0f );
	}

	numGibs = random->RandomInt( 1, NUM_ZOMBIE_GIBS_SMALL );

	// Spawn all the small gibs
	for ( int i = 0; i < NUM_ZOMBIE_GIBS_SMALL; i++ )
	{
		offset = RandomVector( -16, 16 ) + origin;

		C_ZombieGib::CreateClientsideGib( pszZombieGibs_Small[i], offset, ( direction + RandomVector( -0.8f, 0.8f ) ) * ( 400 * scale ), RandomAngularImpulse( -300, 300 ), 0.5f );
	}
	...
	CSmartPtr<CSimpleEmitter> pSimple = CSimpleEmitter::Create( "FX_ZombieGib" );
	pSimple->SetSortOrigin( origin );
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void ZombieGibCallback( const CEffectData &data )
{
	FX_ZombieGib( data.m_vOrigin, data.m_vNormal, data.m_flScale );
}

DECLARE_CLIENT_EFFECT( "ZombieGib", ZombieGibCallback );
