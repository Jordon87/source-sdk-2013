...
class CHudTorch : public CHudElement, public vgui::Panel
{
 
	DECLARE_CLASS_SIMPLE(CHudTorch, vgui::Panel);
 
public:
	CHudTorch(const char * pElementName);
...
CHudTorch::CHudTorch (const char * pElementName) : 
	CHudElement (pElementName), BaseClass (NULL, "HudTorch")
{
	vgui:: Panel * pParent = g_pClientMode-> GetViewport ();
	SetParent (pParent);

	m_nTexture_BG = surface()->CreateNewTextureID();
	surface()->DrawSetTextureFile( m_nTexture_BG, "vgui/hud/torch_bg", true, false );

	m_nTexture_FG = surface()->CreateNewTextureID();
	surface()->DrawSetTextureFile( m_nTexture_FG, "vgui/hud/torch_fg", true, false );

	m_nTexture_On = surface()->CreateNewTextureID();
	surface()->DrawSetTextureFile( m_nTexture_On, "vgui/hud/torch_on", true, false );

	m_nTexture_Off = surface()->CreateNewTextureID();
	surface()->DrawSetTextureFile( m_nTexture_Off, "vgui/hud/torch_off", true, false );
	...
	SetHiddenBits (HIDEHUD_FLASHLIGHT | HIDEHUD_PLAYERDEAD | HIDEHUD_NEEDSUIT);
}
...
void CHudTorch:: OnThink (void)
{
	...
	newHull = pPlayer->m_HL2Local.m_flFlashBattery;
	m_bIsOn = pPlayer->IsEffectActive( EF_DIMLIGHT );
...
void CHudTorch::Paint()
{
	...
	if (m_bIsOn)
	{
		surface()->DrawSetTexture( m_nTexture_On );
	}
	else
	{
		surface()->DrawSetTexture( m_nTexture_Off );
	}
...
void CHudTorch::PaintBackground()
{
